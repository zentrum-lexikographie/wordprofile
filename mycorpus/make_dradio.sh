...
DYNET_SEED=14464227
IMSPARSER=$zdl_scripts/ims_parser/parse.py
...
dta-cab-analyze.perl \
	...
	-jobs=$NJOBS -block=16k@s \
    -ifc=xmltokwrapfast dradio/dradio.list \
...
python3 $zdl_scripts/tj2conllu.py dradio/dradio.tj.list --list --jobs $NJOBS
...
python3 $IMSPARSER \
    ...
    --parser TRANS \
    --format conllu \
    --src dradio/dradio.conllu.list --list --jobs $NJOBS

python3 $zdl_scripts/conllu2rel.py "dradio/id_*.trans.conllu" dradio_rels --jobs $NJOBS