...
"""
  Einfärbungen für die farbliche Kennzeichnung beim Wortvergleich

"""
def black(x):
  if type(x) == type(99) or type(x) == type(9.9):
    x = str(x)
...
parser = OptionParser()
parser.add_option("--l1", dest="lemma1", default=None, help=u"das Eingabewort")
parser.add_option("--l2", dest="lemma2", default=None, help=u"das Eingabewort")
parser.add_option("-n", dest="number", default=20, help=u"Anzahl der Relationstupel (default=20)")
parser.add_option("-b", dest="nbest", default=-1, help=u"Die Anzahl der zu vergleichenden Relationstupel beider Wörter von vornherein einschränken")
...
parser.add_option("-c", dest="corpus", default="", help=u"Angabe des korpus (zeit,kern,21jhd)")
...
parser.add_option("-o", dest="order", default="logDice", help=u"Angabe der Ordnung (frequency,logDice,MiLogFreq,MI3) (default=logDice)")
#parser.add_option("--is",action="store_true", dest="intersection", default=False, help=u"Schnitt berechen")
parser.add_option("--op", dest="operation", default="adiff", help=u"Operation (adiff,rmax), Default: adiff") #diff,adiff,max,min,rmax,avg,havg,gavg
...
if len(mapping) > 0:  
  ...
  if len(mapping) == 1:
    mapSelect = mapping[0]
    print "\033[32;1m"+"(1) "+"\033[m" + yellow(mapSelect["Lemma1"])+"/"+ cyan(mapSelect["Lemma2"]) + " [" + mapSelect["POS"]+"]"
  else:
    ...
    while True:
      iCounter=1
      for i in mapping:
        print "\033[32;1m"+"("+str(iCounter)+") "+"\033[m" + yellow(i["Lemma1"])+"/"+ cyan(i["Lemma2"]) + " [" + i["POS"]+"]"
        iCounter = iCounter+1
