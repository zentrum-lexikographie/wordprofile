...
parser = OptionParser()
parser.add_option("-i", dest="info", default=-1, help=u"die Texttreffer-ID")
parser.add_option("-s", dest="start", default=0, help=u"Trefferstart")
parser.add_option("-n", dest="number", default=20, help=u"Trefferanzahl (default=20)")
parser.add_option("-x", dest="host", default="http://localhost:9999", help=u"host default=http://services.dwds.de:9999")
parser.add_option("-u",action="store_true", dest="internal_user", default=False, help=u"interner Benutzer (mit Rechten)")
parser.add_option("--ct",action="store_true", dest="context", default=False, help=u"anzeigen der Contexte (rechter, linker Satz)")
parser.add_option("--br", dest="width", default=140, help=u"Breite der Anzeige (default=140)")
parser.add_option("--sc",action="store_true", dest="score", default=False, help=u"primär nach dem Sentence-Score sortieren")
parser.add_option("-a",action="store_false", dest="dateDesc", default=True, help=u"Datum aufsteigend")
parser.add_option("-c", dest="corpus", default="", help=u"einzelner Korpus, in dem gesucht werden soll")
...
for i in mapRelInfo['Tuples']:
  ...
  if strFile == "":
    ...
    strMeta =  "%i) %s | %s | %s | %s | %s | %s | %s | %s" % (iCounter, strBiblCorpus, strBiblDate, strBiblTextclass, strBiblOrig, strBiblScan, strBiblAvail, strBiblPage, strScore)
    listConcord.append((strLeft + strSentence + strRight,strMeta.decode('utf8')))
  else:
