...
def create_tables(cursor,directory):
  ...
  cursor.execute ("LOAD DATA " + g_strLocal + " INFILE \""+os.path.realpath(directory +"/TEI_types.table")+"\" INTO TABLE teiTypes")
  cursor = conn.cursor ()
...
def load_into_tables(cursor,directory):
  ...
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/mapping_TEI_date.table')+'\" INTO TABLE idToDate')
  print '(: load data: '+ os.path.realpath(directory +'/mapping_TEI_orig.table')
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/mapping_TEI_orig.table')+'\" INTO TABLE idToOrig')
  print '(: load data: '+ os.path.realpath(directory +'/mapping_TEI_scan.table')
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/mapping_TEI_scan.table')+'\" INTO TABLE idToScan')
  print '(: load data: '+ os.path.realpath(directory +'/mapping_TEI_avail.table')
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/mapping_TEI_avail.table')+'\" INTO TABLE idToAvail')
  print '(: load data: '+ os.path.realpath(directory +'/mapping_TEI_textclass.table')
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/mapping_TEI_textclass.table')+'\" INTO TABLE idToTextclass')
  print '(: load data: '+ os.path.realpath(directory +'/concord_sentences.table')
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/concord_sentences.table')+"\" INTO TABLE concordSentences FIELDS TERMINATED BY '\t' ENCLOSED BY '' ESCAPED BY ''")
  print '(: load data: '+ os.path.realpath(directory +'/mapping_position_info_tei.table')
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/mapping_position_info_tei.table')+'\" INTO TABLE idToInfo')
  print '(: load data: '+ os.path.realpath(directory +'/mapping_TEI.table')
  cursor.execute ('LOAD DATA ' + g_strLocal + ' INFILE \"'+os.path.realpath(directory+'/mapping_TEI.table')+'\" INTO TABLE idToTei')
  
print "|: UPDATE DATABASE"

# Create option parser
parser = OptionParser()
parser.add_option("-s", dest="spec", default="query-specification.xml", help=u"Angabe der Settings-Datei (*.xml)")
