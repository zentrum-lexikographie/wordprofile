...
from wpse_mysql import *
from wpse_tree import *
from wpse_string import *
from wpse_spec import *
import os
...
"""
  ...
  *Es können Wortprofile miteinander Verglichen werden. 
"""
class WortprofilQuery (SimpleXMLRPCServer.SimpleXMLRPCRequestHandler):
  ...
  def gen_rel_ids_by_rel ( self, listRel ):
    setRes=set()
    for i in listRel:
      if self.CWpMySQL.mapRelToId.has_key(i):
  ...
  def gen_rel_ids_by_rel_and_pos ( self, listRel, strPOS ):
    ...
    if self.CWpSpec.mapRelOrder.has_key(strPOS):
  ...
  def gen_rel_ids_by_pos ( self, strPOS ):
    ...
    if self.CWpSpec.mapMweRelOrder.has_key(strPOS):
  ...
  def __gen_rel_pos_cooccurrence_mapping( self, listData ):
    ...
    for i in listData:
      if (i[xLemma1],i[xPos1]) in mapRes:
        if mapRes[(i[xLemma1],i[xPos1])].has_key(i[xRel]):
  ...
  def get_lemma_and_pos_by_list( self, mapParam ):
    ...
    if mapParam.has_key("CaseSensitive"):
  ...
  def get_lemma_and_pos( self, mapParam ):
    ...
    ### Parameter
    if mapParam.has_key("UseVariations"):
    ...
    if listRes == [] and bool(bUseExternalVariations):
      ...
      if self.CWpSpec.mapVariation.has_key(strWord):
  ...
  def __get_lemma_and_pos_base( self, mapParam ):
      ...
      if type(strWord) == type(u''):
        strUnicodeWord = strWord
        strWord = strWord.encode("utf8")
      else:
        strUnicodeWord = unicode(strWord.decode('utf8'))
  ...
  def __check_and_sort_lemma_and_pos_list( self, listLemmaPos ,strUnicodeWord, bCaseSensitive ):
      ...
      for i in vSort:
        ...
        strUnicode2 = unicode(strLemma.decode('utf8'))
        ...
        if strUnicode1 == strUnicode2 and bInputIsUpper1 and bInputIsUpper2:
          if strPos=="Substantiv":
            listRes1a.append({'LemmaId':iLemma,'PosId':iPos,'POS':strPos,'Lemma':strLemma,'Frequency':iFrequency,'Count':iCount,'Relations':listRelation})
          else:
            listRes1b.append({'LemmaId':iLemma,'PosId':iPos,'POS':strPos,'Lemma':strLemma,'Frequency':iFrequency,'Count':iCount,'Relations':listRelation})
        elif strUnicode1.lower() == strUnicode2.lower():
          listRes2.append({'LemmaId':iLemma,'PosId':iPos,'POS':strPos,'Lemma':strLemma,'Frequency':iFrequency,'Count':iCount,'Relations':listRelation})
        elif bInputIsUpper1 == bInputIsUpper2:
          listRes3.append({'LemmaId':iLemma,'PosId':iPos,'POS':strPos,'Lemma':strLemma,'Frequency':iFrequency,'Count':iCount,'Relations':listRelation})
        else:
          listRes4.append({'LemmaId':iLemma,'PosId':iPos,'POS':strPos,'Lemma':strLemma,'Frequency':iFrequency,'Count':iCount,'Relations':listRelation})
...
[ {'Lemma1':<string>,'Lemma2':<string>,'POS':<string>,'LemmaId1':<int>,'LemmaId2':<int>,'PosId':<int>,'Frequency1':<int>,'Frequency2':<int>,'Count1':<int>,'Count2':<int>,'Relations:<Liste aus Strings>} , ... ]
  ...
  def get_lemma_and_pos_diff( self, mapParam ):
    ...
    if list1 == [] and bool(bUseExternalVariations):
      ...
      if self.CWpSpec.mapVariation.has_key(strLemma):
    ...
    for i in list1:
      for j in list2:
        if i['PosId'] == j['PosId']:
          ...
          listResult.append({'Lemma1':i['Lemma'], 'Lemma2':j['Lemma'],'LemmaId1':i['LemmaId'], 'LemmaId2':j['LemmaId'],'PosId':i['PosId'],'POS':i['POS'],'Frequency1':i['Frequency'],'Frequency2':j['Frequency'],'Count1':i['Count'],'Count2':j['Count'],'Relations':listRelations})
  ...
  def get_relations( self, mapParam ):
    ...
    for i in listSortId:
      ...
      if self.CWpSpec.mapRelDesc.has_key(strRel):
  ...
  def __get_relation_tuples_mwe_check( self, mapParam, listRelationId ):
    ...
    if mapParam.has_key("Start"):
      iStart = mapParam["Start"]
    if mapParam.has_key("Number"):
      iNumber = mapParam["Number"]
    if mapParam.has_key("OrderBy"):
      strOrderBy = mapParam["OrderBy"]
    if mapParam.has_key("MinFreq"):
      iMinFreq = mapParam["MinFreq"]
    if mapParam.has_key("MinStat"):
    ...
    if iPos2ID==-1 or iLemma2ID==-1:
      strWhere = "WHERE lemma1=\""+str(iLemmaID)+"\" and POS1=\""+str(iPosID)+"\" and function IN "+strRelIds+" "+strMinFreq+" "+strMinStat+" LIMIT "+str(iStart)+", "+str(iNumber)+";"
    else:
      strWhere = "WHERE lemma1=\""+str(iLemmaID)+"\" and POS1=\""+str(iPosID)+"\" and lemma2=\""+str(iLemma2ID)+"\" and POS2=\""+str(iPos2ID)+"\" and function IN "+strRelIds+" "+strMinFreq+" "+strMinStat+" ORDER BY frequency;"
  ...
  def __relation_tuples_2_strings(self,listCooccTupel,mapParam):
    ...
    for i in listCooccTupel:
      ...
      strSurface=self.CWpStr.surface_mapping(strSurface, localMap['Relation'],self.CWpMySQL.mapRelIdToType[i[xRel]],strPrepSurface,bExtendedSurfaceForm)
      ...
      localMap["ConcordId"] = str(mapParam["LemmaId"])+"#"+str(mapParam["PosId"])+"#"+str(i[xLemma2])+"#"+str(i[xPOS])+"#"+str(i[xPrep])+"#" +str(i[xRel])+'#'+str(i[xInfo])
  ...
  def __mwe_relation_tuples_2_strings(self,listData,listCooccRef,mapParam,strInfoId):
    ...
    for k in listCooccRef:
      ...
      mapScore['MI3']=i[xScore_MI3]
      localMap['Score'] = mapScore
      localMap["ConcordId"] = strMweId+"@"+str(i[xLemma1])+"#"+str(i[xPOS1])+"#"+str(i[xLemma2])+"#"+str(i[xPOS2])+"#"+str(i[xPrep])+"#"+str(i[xRel])+'#'+str(i[xInfo])
  ...
  def get_mwe_relations_by_list( self, mapParam ):
    ...
    listInfo = self.CWpTree.lemma_and_pos_list_to_tree(mapParam['Parts'], self.CWpMySQL.mapRelToId, self.CWpMySQL.mapRelIdToType)
    if len(listInfo)==0:
      return {'data':[],'parts':{}}
    ...
    for j in mapRelDataRef:
      ...
      for i in listLocalRelId:
        if mapRelDataRef[j].has_key(i):
          ...
          if mapResultPerLemma.has_key(strLem):
            mapResultPerLemma[strLem].append({'Relation':strRel,'Description':strDesc,'Tuples':listTuples,'RelId':strRelId}) 
          else:
            mapResultPerLemma[strLem]=[{'Relation':strRel,'Description':strDesc,'Tuples':listTuples,'RelId':strRelId}]
  ...
  def get_mwe_relations_by_list_parametric( self, mapParam ):
    ...
    for i in listLemma:
      ...
      if i['Relation']=="*":
        ...
        vRel = self.CWpTree.rellist_to_idlist_directed ( mapping1[0]['Relations'], self.CWpMySQL.mapRelToId, self.CWpMySQL.mapRelIdToType )
      else:
        ### bestimmte Relation
  ...
  def __get_mwe_rec_select( self, iStelligkeit ):
    ...
    for i in range(1,iMax+1):
      if i==1:
        ...
        strCheckRec=",myMweList"+str(iStelligkeit)+".id"+str(i-1)+",myMweList"+str(iStelligkeit)+".id"+str(i)

      if strConditionalRec!="":
        strConditionalRec+=" , "
      if i==1:
        strConditionalRec+="func_order_initial(idToConditional_"+str(iStelligkeit)+".mate"+strCheckRec+")"
        pass
      elif i == iMax:
        strConditionalRec+="func_order_final(idToConditional_"+str(iStelligkeit)+".mate"+strCheckRec+")"
        pass
      else:    
        strConditionalRec+="func_order_middle(idToConditional_"+str(iStelligkeit)+".mate"+strCheckRec+")"
        pass
      iCount+=1
  ...
  def __get_mwe_rec_select_free( self, iStelligkeit ):
    ...
    for i in range(1,iMax+1):
      ...
      if strConditionalRec!="":
        strConditionalRec+=" , "
      if i==1:
        strConditionalRec+="func_order_initial(idToConditionalFree_"+str(iStelligkeit)+".mate"+strCheckRec+")"
        pass
      elif i == iMax:
        strConditionalRec+="func_order_final(idToConditionalFree_"+str(iStelligkeit)+".mate"+strCheckRec+")"
        pass
      else:    
        strConditionalRec+="func_order_middle(idToConditionalFree_"+str(iStelligkeit)+".mate"+strCheckRec+")"
        pass
      iCount+=1
  ...
  def __get_mwe_rec_join_on( self, iStelligkeit ):
    ...
    for i in range(1,iMax+1):
      if strJoinOn != "":
        strJoinOn += " and "
      strJoinOn += "myMweList"+str(iStelligkeit)+".id"+str(i)+" = idToConditional_"+str(iStelligkeit)+".id"+str(i)+" "
  ...
  def __get_mwe_rec_join_on_free( self, iStelligkeit ):
    ...
    for i in range(1,iMax+1):
      if strJoinOn != "":
        strJoinOn += " and "
      strJoinOn += "myMweList"+str(iStelligkeit)+".id"+str(i)+" = idToConditionalFree_"+str(iStelligkeit)+".id"+str(i)+" "
  ...
  def get_mwe_relations_by_list_base ( self, mapParam, listMweRec ):
    ...
    strInitialInfo = "CONCAT('%s#%s#%s#%s#',%s,'#',%s,'#',%s)" % (str(iLemmaID),str(iPosID),str(iLemma2ID),str(iPos2ID),"relations.prep","relations.function","relations.info")
    ...
    strWhere1 = "WHERE lemma1=\"%s\" and POS1=\"%s\" and lemma2=\"%s\" and POS2=\"%s\" and function IN %s %s;" % (listMweRec[0][0]["LemmaId"],listMweRec[0][0]["PosId"],listMweRec[0][1]["LemmaId"],listMweRec[0][1]["PosId"],strRelIds,strWherePrep)
    ...
    for i in range(1,len(listMweRec)):
      ...
      ### Zusammenbauen der MWE-ID für spätere Abfragen
      strInfo2 = "CONCAT(myMweList%i.info,'@',idToConditionalFree_%i.lemma1,'#',idToConditionalFree_%i.pos1,'#',idToConditionalFree_%i.lemma2,'#',idToConditionalFree_%i.pos2,'#',idToConditionalFree_%i.prep,'#',idToConditionalFree_%i.function,'#',idToConditionalFree_%i.mate)" % (iCount,iCount,iCount,iCount,iCount,iCount,iCount,iCount)

      ### mit Hilfe der Tabelle 'tmpMweList' die Treffer-IDs ermitteln
      strCreate2 = "CREATE TEMPORARY TABLE myMweList"+str(iCount+1)+" LIKE tmpMweList"+str(iCount+1)+"; "
      strIn2 = "INSERT INTO myMweList"+str(iCount+1)+" "
      strSelect2 = "SELECT  "+strRecSelect + ", " + strInfo2 +" "
      strFrom2= "FROM myMweList"+str(iCount)+" STRAIGHT_JOIN idToConditionalFree_"+str(iCount)+" ON ( "+strRecJoinOn+" )"
      strWhere = " WHERE (lemma1=\"%s\" and POS1=\"%s\") and (lemma2=\"%s\" and POS2=\"%s\") and function IN %s %s " % (listMweRec[i][0]["LemmaId"],listMweRec[i][0]["PosId"],listMweRec[i][1]["LemmaId"],listMweRec[i][1]["PosId"],strRelIds,strWherePrep)
    ...
    strInfoCut = "CONCAT(myMweList%i.info,'@',idToConditional_%i.lemma,'#',idToConditional_%i.pos,'#0#',idToConditional_%i.function,'#',idToConditional_%i.mate)" % (iCount,iCount,iCount,iCount,iCount)
    ...
    strSelectCut = " SELECT idToConditional_"+str(iCount)+".mate,-idToConditional_"+str(iCount)+".frequency,-idToConditional_"+str(iCount)+".freqBelege,-idToConditional_"+str(iCount)+".logDice,idToConditional_"+str(iCount)+".function,idToConditional_"+str(iCount)+".lemma,idToConditional_"+str(iCount)+".POS, "+strRecSelect+",  myMweList"+str(iCount)+".info "
    strFromCut= "FROM myMweList"+str(iCount)+" STRAIGHT_JOIN idToConditional_"+str(iCount)+" USE INDEX(I_"+strOrderBy+") ON ( "+strRecJoinOn+" )"
    strWhereCut = " WHERE idToConditional_"+str(iCount)+".lemma=$LEMMA$ and idToConditional_"+str(iCount)+".POS=$POS$ and idToConditional_"+str(iCount)+".function=$FUNCTION$ and -idToConditional_"+str(iCount)+".logDice>="+strMinStat+" and -idToConditional_"+str(iCount)+".frequency>="+strMinFreq+" "
    ...
    for i in listMweRec:
      if (i[0]["LemmaId"],i[0]["PosId"]) not in mapDone:
        for j in i[0]["Relations"]:   
          if len(setRelations)==0 or j in setRelations:
            strWhereCutDummy = strWhereCut.replace("$LEMMA$",str(i[0]["LemmaId"])).replace("$POS$",str(i[0]["PosId"])).replace("$FUNCTION$",str(self.CWpMySQL.mapRelToId[j]))
      ...
      if (i[1]["LemmaId"],i[1]["PosId"]) not in mapDone:
        for j in i[1]["Relations"]:   
          if len(setRelations)==0 or j in setRelations:
            strWhereCutDummy = strWhereCut.replace("$LEMMA$",str(i[1]["LemmaId"])).replace("$POS$",str(i[1]["PosId"])).replace("$FUNCTION$",str(self.CWpMySQL.mapRelToId[j]))
            self.CWpMySQL.execute( strInCut+strSelectCut+strFromCut+strWhereCutDummy+strLimitCut )
    ...
    for i in range(0,iCount+1):
      ...
      strConditionalRecJoin+="ConditionalCheck_"+str(iCount+1)+".id"+str(iCount2)+"=myMateCut.id"+str(iCount2)
      iCount2+=1
...
if(ConditionalCheck_"""+str(iCount+1)+""".id1!=CAST('None' as UNSIGNED) """+strMinFreqCheck+strMinStatCheck+""",1,0),myMateCut.info """
    ...
    for j in mapRelData:
      ...
      for i in listLocalRelId:
        if mapRelData[j].has_key(i):
          ...
          strRelId = mapParam['ConcordIdRemember']+"@"+str(j[0])+'#'+str(j[1])+'#'+str(i)#strInfoId
  ...
  def __get_relation_tuples_mwe_single ( self, mapParam ,setInfoId,mapLemCat):
    ...
    for i in range(0,len(setInfoId)+1):
      ...
      if strConditionalRec!="":
        strConditionalRec+=" , "
      if i==0:
        strConditionalRec+="func_order_initial"+"(idToConditional_"+str(iMweStelligkeit)+".mate"+strCheckRec+")"
        pass
      elif i == len(setInfoId):
        strConditionalRec+="func_order_final"+"(idToConditional_"+str(iMweStelligkeit)+".mate"+strCheckRec+")"
        pass
      else:    
        strConditionalRec+="func_order_middle"+"(idToConditional_"+str(iMweStelligkeit)+".mate"+strCheckRec+")"
        pass
      iCount+=1
    strConditionalRecJoin=""
    iCount=1
    for i in range(0,len(setInfoId)+1):
      if strConditionalRecJoin!="":
        strConditionalRecJoin+=" and "
      strConditionalRecJoin+="ConditionalCheck_"+str(iMweStelligkeit+1)+".id"+str(iCount)+"=myMateCut.id"+str(iCount)
      iCount+=1
    ...
    strSelect1 = " SELECT idToConditional_"+str(iMweStelligkeit)+".mate,-idToConditional_"+str(iMweStelligkeit)+".frequency,-idToConditional_"+str(iMweStelligkeit)+".freqBelege,-idToConditional_"+str(iMweStelligkeit)+".logDice,idToConditional_"+str(iMweStelligkeit)+".function,idToConditional_"+str(iMweStelligkeit)+".lemma,idToConditional_"+str(iMweStelligkeit)+".POS,"+strConditionalRec+", 0 "
    strFrom1= "FROM idToConditional_"+str(iMweStelligkeit)+" USE INDEX(I_"+strOrderBy+") "
    strWhere1 = " WHERE "+strWhereRec + " and idToConditional_"+str(iMweStelligkeit)+".lemma=$LEMMA$ and idToConditional_"+str(iMweStelligkeit)+".POS=$POS$ and idToConditional_"+str(iMweStelligkeit)+".function=$FUNCTION$ and -idToConditional_"+str(iMweStelligkeit)+".logDice>="+strMinStat+" and -idToConditional_"+str(iMweStelligkeit)+".frequency>="+strMinFreq+" "
    ...
    if self.CWpMySQL.iMweDepth>iMweStelligkeit:
      ...
      strMinFreqCheck = ""
      if iMinFreq > 0:
        strMinFreqCheck = " and (-ConditionalCheck_"+str(iMweStelligkeit+1)+".frequency)>="+str(iMinFreq)+" "
      strMinStatCheck = ""
      if iMinStat > -100000000:
        strMinStatCheck = " and (-ConditionalCheck_"+str(iMweStelligkeit+1)+".logDice)>="+str(iMinStat)+" "
...
if(ConditionalCheck_"""+str(iMweStelligkeit+1)+""".id1!=CAST('None' as UNSIGNED) """+strMinFreqCheck+strMinStatCheck+""",1,0) """
      strFrom3= """FROM myMateCut 
                    STRAIGHT_JOIN relations FORCE INDEX(I_info) ON 
                    (
                      relations.info=myMateCut.mate and 
    ...
    for i in mapLemCat:
      ...
      for j in listRelId:
        strWhereDummy = strWhere1.replace("$LEMMA$",str(i[0])).replace("$POS$",str(i[1])).replace("$FUNCTION$",str(j))
  ...
  def __get_relation_tuples_mwe( self, mapParam ,listRelationId,setInfoId):
    ...
    if len(listRelationId)>0:
      strRelIds = " and idToConditional_"+str(iMweStelligkeit)+".function IN"+self.CWpMySQL.list_2_in(listRelationId)
    iCount=1
    for i in range(0,len(setInfoId)+1):
      if i==0:
        strCheckRec=','+str(vCheckRec[i])
    ...
    strSelect1 = " SELECT idToConditional_"+str(iMweStelligkeit)+".mate,-idToConditional_"+str(iMweStelligkeit)+".frequency,-idToConditional_"+str(iMweStelligkeit)+".freqBelege,-idToConditional_"+str(iMweStelligkeit)+".logDice,idToConditional_"+str(iMweStelligkeit)+".function,idToConditional_"+str(iMweStelligkeit)+".lemma,idToConditional_"+str(iMweStelligkeit)+".POS "
    ...
    strWhere2 = " WHERE x.logDice>="+strMinStat+" and frequency>="+strMinFreq+" and x.row_number>"+str(iStart)+" and x.row_number <= "+str(iNumber+iStart)+";"
...
if(ConditionalCheck_"""+str(iMweStelligkeit+1)+""".id1!=CAST('None' as UNSIGNED) """+strMinFreqCheck+strMinStatCheck+""",1,0) """
  ...
  def get_relation_tuples_diff( self, mapParam, listRelationId ):
    ...
    strSelect = "SELECT function, prep, lemma1, lemma2, surfacePrep, surface1, surface2, POS2, -frequency, -freqBelege, -"+strOrderBy+", info  FROM "+strSubcorpus+"relations  USE INDEX(I_"+strOrderBy+") WHERE lemma1 IN (\""+str(iLemmaId1)+"\",\""+str(iLemmaId2)+"\") and POS1=\""+str(iPosID)+"\" and function IN "+strRelIds+" "+strMinFreq+" "+strMinStat+";"
  ...
  def __calculate_diff(self,iRel,listData,listRelData,mapParam):
    ...
    for k in listRelData:
      ...
      if i[xLemma1] == iLem1:
        if iCountRank1 <= iNBest or iNBest==None:
          if mapData.has_key((i[xPrep],i[xLemma2])):
    ...
    if bIntersect:
      ...
      for i in mapCenter:
        ...
        if iRef != -1:
          ...
          iScore = self.__diff_operation(strOperation,listData[iRef1][xScore],listData[iRef2][xScore],iRank1,iRank2)
    ...
    abs_lcmp = lambda idx: lambda i, j: (math.fabs(i[idx]) > math.fabs(j[idx])) and -1 or (math.fabs(i[idx]) < math.fabs(j[idx])) and 1 or 0
    rcmp = lambda idx: lambda i, j: (i[idx] < j[idx]) and -1 or (i[idx] > j[idx]) and 1 or 0
    abs_rcmp = lambda idx: lambda i, j: (math.fabs(i[idx]) < math.fabs(j[idx])) and -1 or (math.fabs(i[idx]) > math.fabs(j[idx])) and 1 or 0
  ...
  def __diff_relation_tuples_2_strings(self,listData,listRes,mapParam):
    ...
    for i in listRes:
      ...
      if i[yRef1] != -1:
        ...
        if self.CWpMySQL.mapRelIdToType[listData[i[yRef1]][xRel]] == 2 and listData[i[yRef1]][xLemma1] == listData[i[yRef1]][xLemma2]:
        ...
        ###Oberflächenform formatieren (z.B. bei erweiterten Oberflächenformen mit Kontext)
        strSurface=self.CWpStr.surface_mapping(strSurface, localMap['Relation'],self.CWpMySQL.mapRelIdToType[listData[i[yRef1]][xRel]],strPrepSurface,bExtendedSurfaceForm)
      ...
      if i[yRef2] != -1:
        ...
        if self.CWpMySQL.mapRelIdToType[listData[i[yRef2]][xRel]] == 2 and listData[i[yRef2]][xLemma1] == listData[i[yRef2]][xLemma2]:
        ...
        if i[yRef1] == -1:
          ...
          ###Oberflächenform formatieren (z.B. bei erweiterten Oberflächenformen mit Kontext)
          strSurface=self.CWpStr.surface_mapping(strSurface, localMap['Relation'],self.CWpMySQL.mapRelIdToType[listData[i[yRef2]][xRel]],strPrepSurface,bExtendedSurfaceForm)
...
{'Relation':<string>,'Lemma1':<string>,'Lemma2':<string>,'POS1':<string>,'POS2':<string>,'Form1':<string>,'Form2':<string>} 
  ...
  def get_relation_by_info_id ( self, mapParam ):
    ...
    if self.CWpSpec.mapRelDescDetail.has_key(strRel):
    ...
    return {'Description':strDescLoc,'Relation':strRel,'Lemma1':strLem1,'Lemma2':strLem2,'POS1':strPOS1,'POS2':strPOS2}
  ...
  def __get_concordances_base( self, mapParam ):
    ...
    ### Parameter
    if (mapParam.has_key("UseContext")):      
      bUseContext= mapParam["UseContext"]
    bInternalUser=False
    if (mapParam.has_key("InternalUser")):      
      bInternalUser=bool(mapParam["InternalUser"])
    if (mapParam.has_key("UseScore")):      
      bUseScore= mapParam["UseScore"]
    if (mapParam.has_key("DateDesc")):      
    ...
    strIn = "INSERT INTO relDouble SELECT idToInfo.* FROM idToInfo USE INDEX("+strIndex+") WHERE idToInfo.id=\""+ str(iInfoId) +"\" "+strSubcorpus + strInternalUser + " LIMIT "+str(iStart)+", "+str(iNumber)+" ; "
    ...
    for i in listRes:
      ...
      listMapRes.append({"Bibl":mapBib, "ConcordLine":strCenter, "ConcordLeft":strLeft, "ConcordRight":strRight, "Score":strScore})
  ...
  def __get_concordances_mwe_base( self, mapParam ):
    ...
    strPositionSelect=""
    for i in range(1,len(listInfoId)+1):
      strPositionSelect+=", infoDouble.tokenPosition1_%i, infoDouble.tokenPosition2_%i, infoDouble.prepPosition_%i" % (i,i,i)
    ...
    strFrom1= """FROM idToInfo as infoDouble1 FORCE INDEX("""+strIndex+""") 
                                               """ +strInfoJoin+ """
                                    where ("""+strInfoWhere+""" ) LIMIT """+str(iStart)+','+str(iNumber)+""" ;"""
...
parser.add_option("-p", dest="port", default=None, help=u"Angabe des Ports")
parser.add_option("-r",action="store_false", dest="reload_mmap", default=True, help=u"mmap-Dateien nicht neu berechnen")
parser.add_option("--log", dest="logfile", default=None, help=u"Angabe der log-Datei (Default: log/wp_{date}.log)")
...
# Create server
server = SimpleXMLRPCServer.SimpleXMLRPCServer(("", int(options.port)),
